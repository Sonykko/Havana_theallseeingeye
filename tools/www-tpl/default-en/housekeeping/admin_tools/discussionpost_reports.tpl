{% include "housekeeping/base/header.tpl" %}
  <body>
	...
	{% include "housekeeping/base/navigation_admin_tools.tpl" %}
     <h2 class="mt-4">Discussion Post Reports</h2>		
		...
		<form id="getAllReports" action="{{ site.sitePath }}/{{ site.housekeepingPath }}/admin_tools/discussionpost_reports_list" method="post">
			...
			<table class="table table-striped">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{ contentReportsList.postMessage }}</td>
		...
		{% endif %}			
			...
			<form id="processReportsForm" action="{{ site.sitePath }}/{{ site.housekeepingPath }}/admin_tools/discussionpost_reports_list" method="post">
			...
			<br/>
			<p><b>Note:</b> when you click "Process", Discussion posts whose checkbox has been checked reply will be changed. Other reports are ignored and deleted from database.</p>
		</div>				
		{% endif %}	
      </div>
    </div>
	...
	<script>
		...
		document.getElementById("undo").addEventListener("click", function () {
			document.getElementById("remplacementText").value = ""; // Borra el texto
		});		
		...
		document.getElementById("process").addEventListener("click", function (e) {
			...
			// Obtener los objetos y reportes seleccionados
			...
			// Llenar los campos ocultos con los valores seleccionados
